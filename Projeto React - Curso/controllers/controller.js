...
exports.deleteCliente = async(req, res) => {
    try {
        ...
        res.status(500).json({
            message: "Error -> Can NOT delete a customer with id = " +
        req.params.id,
            error: error.message
        });
    }
...
exports.updateCliente = async(req, res) => {
    try {
        ...
        if(!cliente){
            ...
            res.status(404).json({
                message: "Not Found for updating a customer with id = " + clienteld,
                error: "404"  
            });
        } else {
            ...
            if(!result) {
                res.status(500).json({
                    message: "Error -> Can not update a customer with id = " +
                    req.params.id,
